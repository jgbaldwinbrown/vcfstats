...
def parse_my_args():
    ...
    parser.add_argument("-H", "--header",  help="Location of header file.", required=True)
...
def get_arg_vars(args):
    ...
    return((inconn, col, colnames, cnames, args.header))
...
def vcf2sync(inconn, col, col_names, cnames, outconn, headerpath):
    
    with open(headerpath, "r") as headerconn:
        cols = parse_header(headerconn, col_names, cnames)
...
def main():
    ...
    inconn, cols, col_names, cnames, headerpath = get_arg_vars(args)
    outconn = sys.stdout

    vcf2sync(inconn, cols, col_names, cnames, outconn, headerpath)
